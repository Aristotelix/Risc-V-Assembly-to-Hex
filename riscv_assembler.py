...
def main():
    ...
    parser = argparse.ArgumentParser(description="Convert RISC-V assembly into a 32-bit hex instruction memory file (.hex or .txt).")
    parser.add_argument("input_file", help="Path to the input assembly source file (e.g., input.s)")
    parser.add_argument("output_file", help="Path to the output file (must end in .hex or .txt)")
    ...
    if ext.lower() not in ['.hex', '.txt']:
        print("Error: Output file must have a .hex or .txt extension.")
        sys.exit(1)

    # Read lines from input assembly file
    with open(args.input_file, "r") as f:
        lines = f.readlines()


    with open(args.output_file, "w") as out_file:
        parse(lines, out_file)

    print(f"Assembly complete. Memory file successfully created: '{args.output_file}'")
